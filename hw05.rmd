...
I will attempt to solve using least squares approach. Visually my best guess is
dc = a + b*ln(vel). Even though this is linearalizable, I will try to solve it with the nls function.

```{r}
init.vals <- getInitial(dc ~ SSlogis(vel,alpha,xmid,scale),data=data)
reg.nonlinear <- nls(dc ~ a + b*log(vel),start=c(a=init.vals[[2]],b=init.vals[[2]]))

pred.nonlinear <- predict(reg.nonlinear)
plot(dc ~ vel, main="Nonlinear Regression")
lines(pred.nonlinear,col="blue")
```

These results don't make sense. I am not sure how to work with this function...
...
```{r}
(sort(c("Linear"=rmse.linear, "Polynomial"=rmse.poly,"Dependent"=rmse.dep, "Independent"=rmse.ind)))
```
